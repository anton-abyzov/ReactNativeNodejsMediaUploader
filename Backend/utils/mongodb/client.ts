...
export class MongoDBClient {
  ...
  public insert(collection: string, model: Media, result: (error, data) => void): void {
    this.db.collection(collection).insertOne(model, (error, insert) => {
      return result(error, insert.ops[0]);
    });
  }

  public update(collection: string, objectId: string, model: Media, result: (error, data) => void): void {
